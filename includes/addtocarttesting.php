...
echo "<br/>"; 
print_r($platform_id); //for troubleshooting
echo "<br/>"; 
				...
				if ($item_qty > 0) {
					...
					array_push($_SESSION['cart'], 
													  array(
															...
															'platform_id'=> intval($platform_id[$key]),
...
//IF NO MATCH, THEN ADD ALL ELEMENTS OF NEW ITEM TO SESSION
	...
	//Loop through the cart and check quantities for each item
		$lengthCart = count($_SESSION['cart']);//since using a for loop, need toset a counter.
		...
		if ( is_array($qty)) {
			...
			foreach($qty as $key => $item_qty) {
				...
				if ($item_qty > 0) {
						...
						}//end of forinnerloopthroughcart
				...
				//ALSO CHECK FOR QTY > 0 TO AVOID ADDING 0 ITEM TO CART
				 if ($noMatch == true && $item_qty > 0 ) {
						...
						//for troubleshooting
						echo "<p>no match found above for what's alraedy in cart for THIS ITEM on post, AND mor than ZERO: ADD TO CART</p>";
			...
			}//end of foreach OUTER LOOP loop through QTY on POST
...
echo '<h2><a href="../testing.php">Back Home</a></h1>';
//echo '<h2><a href="../products.php">Back to Products</a></h1>';
echo '<h2><a href="../testingviewcart.php">VIEW CART</a></h1>';
?>
