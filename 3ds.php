...
<title>Game Hunter -- 3DS Games</title>
...
$(document).ready(function(){
	$('#dsNav').show( function(){
		...
		$('#huntingFieldLeft').css('background-color', '#4c3596'),
		$('.thickHR').css('color','#4c3596'),
		$('.thickHR').css('background-color', '#4c3596'),
		$('.productTitle').css('border-bottom-color','#4c3596')});
...
<body id="xbox360Index">
    ...
    <div id="pageTitle">
    	<p id="pageTitleText">3DS</p>
        <ul id="pageBreadcrumb3ds">
        <li>Games</li>
        </ul>
...
#################################################	
	...
	$sql = "SELECT * FROM product, platform, product_platform_lookup
			...
			AND product_platform_lookup.platform_id = 4";
...
### start if $start_number
	if ($start_number >= 0) {
		...
		$sql = "SELECT count(*) 
				...
				WHERE product_platform_lookup.platform_id = 4";
